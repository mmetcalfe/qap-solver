...
loop {
    ...
    // for path in glob::glob("data/qapdata/*.dat").unwrap().filter_map(Result::ok) {
        ...
        let duration = Duration::milliseconds(10000);
        ...
        let mut its_search_result = iteratedtabusearch::IteratedTabuSearch::solve(&problem, duration); // , &solution);
        ...
        let mut bma_search_result = bma::Bma::solve(&problem, duration); // , &solution);
